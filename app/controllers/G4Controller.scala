...
object G4Controller extends Controller {
  ...
  def getAll = DBAction { implicit rs =>
    val data: Iterable[G4Json] = getFormatedForJs(G4Redis getAll GithubRepositoryDAO.getAllFetched)
    Ok(Json.parse(data.toJson.compactPrint))
  }

  private def getFormatedForJs(mapList: List[Map[String, Int]]): Iterable[G4Json] = {
    aggregateStatesAndOccurrences(mapList) map { tuple => G4Json(tuple._1, tuple._2) }
  }

  private def aggregateStatesAndOccurrences(mapList: List[Map[String, Int]]): Map[String, Int] = {
    var res = Map[String, Int]()
    mapList foreach { states =>
      states foreach { tuple =>
